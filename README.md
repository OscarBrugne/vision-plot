...
The system is built following a service-oriented architecture, enabling flexible integration and reuse of shared components. For instance, the SVG processing utilities are shared between the two subprojects, even though their purposes are fundamentally different. This architectural choice also allows for heterogeneous implementations: the SVG Utils module is developed using an object-oriented approach, encapsulating logic into reusable classes, while the Projected Augmented Reality module adopts a functional programming paradigm for real-time image transformation and projection.
...
The Pen Plotter module converts bitmap images into vector representations made of continuous lines of uniform thickness, optimized for physical pen plotting. The system ensures that the generated SVG paths are continuous and aesthetically pleasing, using algorithms such as Hilbert curves, spirals, or sinusoidal patterns, based on the brightness of the pixels in the image. The generated SVG files can be directly convert to G-code for CNC machines.

This is an example of an SVG generated by the Pen Plotter module, using this bitmap image as input:
![Projection before calibration](https://github.com/OscarBrugne/vision-plot/blob/main/static/car.jpg)

This SVG was generated using a one-line spiral with a sinusoidal modulation algorithm:
![Projection after calibration](https://github.com/OscarBrugne/vision-plot/blob/main/static/car_spiral_sinus_one_line.png)^

This one was generated using a one-line Hilbert curve with a sinusoidal modulation algorithm:
...
The Projected Augmented Reality (AR) module uses a projector-camera system to overlay predefined visuals onto real-world planes. The system relies on ArUco markers to detect and calibrate the relative positions and orientations of the projector and the camera. During calibration, physical markers are placed on the target and additional markers are projected by the projector. The camera captures the scene, and computer vision techniques are employed to detect these markers and compute the necessary transformations for accurate projection.

This is an example using four physical ArUco markers placed on a wall, defining the corners of a 1.6m × 0.9m rectangle, and five additional markers projected by the projector (center of each edge and the center of the rectangle).
This is the image captured by the camera at the beginning of the calibration process:
...
This is the image captured by the camera after the calibration has been applied:
![Projection after calibration](https://github.com/OscarBrugne/vision-plot/blob/main/static/projection_after_calibration.png)

This is the image captured by the camera after the calibration process, with the different markers detected by the camera:
